...
//
	...
	{
		GOAL = 0;			// No longer seeking
		behave = EXPLORE;	// the goal.
	}
}
...
/*
					target.x=NMap->Nodes[Cur_node]->x_x+NMap->off_x;	//
					target.y=NMap->Nodes[Cur_node]->y_y+NMap->off_y;	//
//printf("\nSet new target as old node was all worn out");
				}
				else
				{
//printf("\nPlanned new route to new target as old node all worn out");
//					ROUTE = 1;
				}
*/
			}
...
/*				target.x=NMap->Nodes[Cur_node]->x_x+NMap->off_x;	//
				target.y=NMap->Nodes[Cur_node]->y_y+NMap->off_y;	//
//printf("\nSet target because of direction flag");
			}
			else
			{
//printf("\nPlanned new route to new target because of direction flag");
//				ROUTE = 1;
			}
//
*/
...
/*				target.x=NMap->Nodes[Cur_node]->x_x+NMap->off_x;	//
				target.y=NMap->Nodes[Cur_node]->y_y+NMap->off_y;	//
//printf("\nSet new target as position achieved without forming a link");
			}
			else
			{
//printf("\nPlanned new route to new target as position achieved without a new link");
//				ROUTE = 1;
			}
//
*/
...
// Otherwise we have a start and end point and can plan a route between
	...
	while(next != -1)	// As ALL nodes are connected this should never happen!
	{
